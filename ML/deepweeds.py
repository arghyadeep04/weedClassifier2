...
#         # Get the content (binary data) of the response
#         image_data = response.content
#         timestamp = datetime.fromtimestamp(time()).strftime('%Y%m%d-%H%M%S')
#         # Specify the local file path where you want to save the image
#         filename = '{}.jpg'.format(timestamp)
#         local_filename = os.path.join(PREDICT_DIR, filename)

#     # Open the local file and write the image data to it
...
#         print(f"train_list: \n{train_dataframe}, test_list = \n{test_dataframe}, val_list = \n{val_dataframe}")
...
#             print(f"Running global epoch: {global_epoch}")
...
#     filenames = os.listdir(PREDICT_DIR)
...
#         # Load image
#         start_time = time()
#         img = imread(PREDICT_DIR + filenames[i])
#         image = Image.get(PREDICT_DIR + filenames[i])
#         st.image(image)
#         # Resize to 224x224
...
#         print(f"Filename : {filenames[i]} belongs to class {y_pred}")
#         st.write(f"""
#         Filename : {filenames[i]} belongs to class {y_pred}
#         """)
...
#     else:
#         if not model.endswith("hdf5" or 'h5'):
